...
function buildTargetUrl(baseUrl: string, requestUrl: string, pathParts: string[]): URL {
    const incoming = new URL(requestUrl);
    const target = new URL(`/api/v1/${pathParts.join('/')}`, baseUrl);
...
async function proxy(request: NextRequest, ctx: { params: Promise<{ path: string[] }> }): Promise<Response> {
    ...
    const targetUrl = buildTargetUrl(baseUrl, request.url, path);
